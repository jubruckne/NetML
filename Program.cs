...
// Displaying some metadata
    ...
    foreach (var t in gguf.tensors.Values) {
        Console.WriteLine(t);
    }

    var token_embeddings = gguf.load_tensor("token_embd.weight");
    var positional_embeddings = gguf.load_tensor("position_embd.weight");

    token_embeddings.print();
    positional_embeddings.print();
    return;
